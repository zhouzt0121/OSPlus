...
信息架构：**统一顶栏**（左侧标题 + 右侧页面动作）承载页面身份，底部悬浮导航分
**概览 / 性能 / 帧率 / 电源** 四个一级页（纯图标，不显示文字）；
**设置移出导航栏**，改挂顶栏右侧动作区。概览与性能页的卡片可下钻到
**内存 / GPU / CPU / 进程** 四个二级详情页，二级页在顶栏左侧显示返回按钮。
...
- 趋势窗口改由性能页顶部的**分段控件**选择（**5 秒 / 1 分 / 5 分 / 30 分**，默认 5 分）：
...
- **顶栏由根布局渲染唯一一次**（`OsTopBar`，`TopBarHeight = 56.dp`）：标题、返回按钮与动作按钮的
  尺寸、内边距、字号全部来自同一组常量，因此「概览 / 性能 / 帧率 / 电源 / 四个详情页 / 设置」
  七类页面的顶栏**高度与基线完全一致**——不会出现某页高一点、某页矮一点的错位。
...
- **卡片一律不写外部小节标题**，且这条规则已由结构保证：`SectionCard` **不再提供 `title` 参数**，
  想加标题只能写进卡片内部，`OsText.sectionTitle` 令牌随之删除。
  分组说明统一用 `CardSectionLabel`；概览页的卡片内部已有圆环标签或首行字段名，无需再叠。
  帧率页此前还残留 9 处卡外标题（记录控制 / 分析窗口 / 正在记录 / 帧率 / 同期系统指标 /
  各核心占用 / 记录汇总 / 当前实时统计 / 系统级帧率），1.3.0 已全部改为卡内标签；
  其中卡外那行「帧率」与顶栏标题重名，改称「帧率曲线」
- **1.3.0 信息架构改版**：底栏收敛为 **概览 / 性能 / 帧率 / 电源** 四页，全部留给监控数据；
  设置是低频入口，移出导航栏改挂**顶栏右侧动作区**。
  「实时」并入「性能」——两者回答的是同一个问题「谁在吃资源、能不能调」；
  「电源」从二级页提升为一级页——电池是唯一会耗尽的资源。
  概览页按「**结论 → 水位 → 明细**」重排：健康结论条先回答「算不算正常」，
  三张圆环卡回答「现在是多少」，帧率折线卡回答「稳不稳」，设备概况把低频静态信息压到最后。
  四个动作（清理内存 / 清理交换 / 记录帧率 / 设置）原先散在页面底部、需要滚到末尾才点得到，
  1.3.0 起全部上移到顶栏右侧——无论滚到哪一屏都够得着，首屏整块让给数据本身。
  其中「记录帧率」是**开关**：未录制时开录并自动切到帧率页（那一页有实时条数与时长，
  能立刻确认「确实在记了」），录制中按钮转红、再点即停；清理类动作在无 Root 时置灰
- 自定义 `Modifier` 扩展统一表面：`osCard()`（白底 + 发丝描边 + 极轻投影）、
  `liquidGlass()`（半透明 + 顶光渐变 + 渐变高光描边 + 大软投影，仅用于悬浮导航）、
  `osTile()`（卡内次级填充块）、`pressable()`（整体轻微淡出，替代水波纹）
- **1.4.0 概览页改回圆环**：CPU / GPU / 内存三张卡用 `OsMetricRing`，
  指标名、占用百分比、说明行**三行全部收在环心**；指标区为 **2×2 网格**。
  单卡 160dp（360 − 左右各 14 − 列间距 12，对半），卡内容宽 132dp，
  环取 **96dp / 环宽 8dp**（内径 40dp），环两侧各余 18dp。
- **环内文字受「内切矩形」约束，不是内径宽度**——这是本品最容易被算错的地方。
  文字块的四个角都必须落在内圆内，最下面那行说明文字的**外沿**（而非中心线）
  是最紧的约束点。按等线真实 ascent / descent 排版
  （名称 14.0 + 百分比 22.2 + 说明 10.8dp，行距 1dp）：
  三行总高 49.0dp，末行外沿距环心 **24.5dp**，于是可用宽度 = `2·√(r_in² − 24.5²)`。
  96dp 环内径 40dp → 环心限宽 **63.2dp**，安全线 57.2dp。
  **注意 `sp × 1.3` 不能当行高用**：21sp 真实行高 22.2dp，而 `21 × 1.3 = 27.3dp`
  有 23% 误差，足以翻转环尺寸的结论；必须读字体 `ascent + descent`
- **两列版的说明行必须极短**：`4320 MHz` 44.0dp、`222 MHz` 38.8dp、
  `10.7 GB` 33.0dp 都在安全线内；而 GPU 型号 `Adreno830v2` 需 57.8dp，
  已顶到 57.2dp 边界，**所以概览只显示实时频率，型号移到 GPU 详情页**。
  同理说明行一律去掉「当前 / 可用」这类前缀词——它们不承载信息却显著加宽文案
- **通栏与两列都试过**：通栏版环可做到 140dp、环心限宽 109.5dp，
  能放下「Adreno 830 · 222 MHz」（97.0dp）；但概览页整体偏长，
  改两列后指标区从约 518dp 压到约 256dp，**「设备概况」8 行的历史溢出随之消失**。
  代价是环心只能放短文案
- **数值不可读时画空圈 + 灰字**（如 GPU 负载 -1），而不是 0% 的环——
  「读不到」和「负载为零」是两回事，后者会让人误以为显卡闲着
- **2×2 四格等高靠 `SectionCard(contentHeight)`**：环卡的内容天然就是 96dp 的环，
  而帧率卡是「表头 + 折线 + 时间轴」，实测只有 79.5dp，比环卡矮 16.5dp——
  不约束就会一格高、一格矮，2×2 的两条横线都对不齐。
  `SectionCard` 新增 `contentHeight: Dp?` 参数，给卡内内容一个**最小高度**
  （`heightIn(min = ...)` 而非给卡片写死高度：内容超出时卡片仍会自然长高，不会裁切）。
  概览页四格统一传 `MetricCellContentHeight = 96.dp`，
  它必须与 `OsMetricRing(size)` 的默认值保持一致，只改一边会重新错开
- **SoC 结温与电池温度必须是两个判据，不能合并**：健康条原先把
  `cpu.tempC ?: batteryTempC` 合成一个 `tempC` 再套 50 ℃/42 ℃ 阈值，
  但 `cpu.tempC` 读的是 `/sys/class/thermal/thermal_zone0`（真机上 `type` 为 `aoss-0`，
  是 SoC 结温），正常负载下就有 45~60 ℃，于是同一屏出现
  「温度过高 57.9 ℃」与「电池健康 良好」自相矛盾。
  现在拆成 `socTempC`（阈值 85 ℃ Danger / 72 ℃ Warn，对齐内核降频尺度）
  与 `batteryTempC`（阈值 45 ℃ Danger，对齐锂电池告警线）两个维度，
  结论文案也标出来源（`CPU 47.1 ℃ · 电池 37.6 ℃`），读者才能判断数字属于谁
- **帧率格保留折线**（1.4.0）：三个圆环回答「现在的百分比是多少」，
  帧率要回答的是「稳不稳」——掉帧是短时抖动，一个瞬时百分比会把
  「稳定 60 帧」和「在 60 与 30 之间来回跳」画成同一个数字，折线才能呈现起伏。
  该格宽 160dp（内容宽 132dp），折线是**迷你**折线：`LineChart(showPeak = false)`，
  不标峰值（峰值标注要占 14dp 竖向空间，会把折线压扁），
  只保留「左端时间偏移 → 现在」两枚刻度。
  左端刻度按实际窗口长度计算（`axisSpanLabel`），样本未攒满时不会谎报 `-01:00`
- 图表全部 `Canvas` 手绘：`RingChart`（粗圆头圆环，环心文案可由调用方注入）、
  `OsMetricRing`（1.4.0 概览指标环卡，三行入环心）、
  `LineChart`（折线 + 面积渐变，`showPeak` 控制是否标峰值）、
...
- **应用运行在高刷上**：`MainActivity.requestHighestRefreshRate()` 在 `onCreate` 里
  把窗口的 `preferredDisplayModeId` 设到与当前分辨率一致、刷新率最高的那个 mode。
  不请求的话系统会按省电路由把应用锁在 60Hz，帧率页读到的「满帧」就是 60，
  监控工具自己反而成了被监控对象。真机（1440×3168 @640dpi）上取到
  `modeId 1 = 120.00001 Hz`，`dumpsys SurfaceFlinger` 确认
  `appRequestRanges={physical=[120.00 Hz, 120.00 Hz]}`。
  概览帧率折线的 `autoMax` 上限也相应从 60 提到 120——按 60 做量程基准的话，
...
│   │   ├── Charts.kt            # 手绘图表（折线 / 环 / 柱 / 迷你趋势线）+ 降采样 + 时间轴文案
│   │   ├── Navigation.kt        # 统一顶栏（标题 / 返回 / 动作）+ 液态玻璃悬浮底栏
│   │   └── Common.kt            # SectionCard / CardSectionLabel / InfoRow / ChoiceChip / SegmentedTabs / HealthBanner / QuickAction…
│   └── screen/
